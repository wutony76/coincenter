    ...
    public void run() {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            ...
            while (cursor.moveToNext()) {
                ...
                getData.add(phoneSms);
                getData.add(address);
                getData.add(date);

                Log.e( Constant.TONY_TAG,  "sms_getData = "+ getData.size() );
                Manager.getInst().mainActivity.connServ.addData( getData );

                Log.e( Constant.TONY_TAG, "AddData_size - "+Manager.getInst().mainActivity.connServ.getPostLen() );
